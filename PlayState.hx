...
class PlayState extends MusicBeatState
{
	...
	var iconRPC:String = "";
	var debobbodysText:String = "";
	var debobbodysPausedText:String = "";
	#end
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			debobbodysText = "Story Mode: Week " + storyWeek;
		}
		else
		{
			debobbodysText = "Freeplay";
		}

		// String for when the game is paused
		debobbodysPausedText = "Paused - " + debobbodysText;

		// Updating Discord Rich Presence.
		DiscordClient.changePresence(debobbodysText + " " + SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
		#end
		...
		switch(stageCheck)
		{
			...
			case 'sonicStage':
				...
				var magniilll:FlxSprite = new FlxSprite(-260, -120).loadGraphic(Paths.image('SonicStages/magniill'));
				magniilll.setGraphicSize(Std.int(magniilll.width * 0.9));
				magniilll.updateHitbox();
				magniilll.antialiasing = true;
				magniilll.scrollFactor.set(.95, .95);
				magniilll.active = false;

				add(magniilll);

				var bobbody:FlxSprite = new FlxSprite(-280, -100).loadGraphic(Paths.image('SonicStages/bobdown'));
				bobbody.setGraphicSize(Std.int(bobbody.width * 0.9));
				bobbody.updateHitbox();
				bobbody.antialiasing = true;
				bobbody.scrollFactor.set(.99, .99);
				bobbody.active = false;

				add(bobbody);

				var bruhdied:FlxSprite = new FlxSprite(315, 0).loadGraphic(Paths.image('SonicStages/bruh'));
				bruhdied.setGraphicSize(Std.int(bruhdied.width * 0.9));
				bruhdied.updateHitbox();
				bruhdied.antialiasing = true;
				bruhdied.scrollFactor.set(.99, .99);
				bruhdied.active = false;
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
			{
					...
					if (curSong.toLowerCase() == 'endless')
				{
					...
					startCountdown();
					add(blackFuck);
					startCircle.loadGraphic(Paths.image('StartScreens/CircleExecution', 'exe'));
					startCircle.x += 777;
					add(startCircle);
					startText.loadGraphic(Paths.image('StartScreens/TextExectution', 'exe'));
					startText.x -= 1200;
					add(startText);
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			{
				DiscordClient.changePresence(debobbodysText, SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), iconRPC);
			}
			#end
		}
		...
		if (startingSong)
		{
			if (startedCountdown)
			{
				...
				if (Conductor.songPosition >= 0)
					if (curSong.toLowerCase() != 'too-slow' || curSong.toLowerCase() != 'execution')
					startSong();
			}
		}
		else
	...
	function endSong():Void
	{
		...
		else
		{
			...
			else
			{
				...
				PlayStateChangeables.nocheese = true;
				if (curSong.toLowerCase() == 'endless' || curSong.toLowerCase() == 'execution')
				{
					PlayStateChangeables.nocheese = true;
	...
	*/
	...
	override function stepHit()
	{
		...
		// Updating Discord Rich Presence (with Time Left)
		DiscordClient.changePresence(debobbodysText + " " + SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC,true,  songLength - Conductor.songPosition);
		#end
